...
from repositories.chat_attachments import (
    ...
    list_chat_attachment_paths_for_document,
...
from repositories.lecture_notes_queue import (
    clear_waiting_lecture_notes_queue,
    count_lecture_notes_queue_by_status,
    enqueue_pages_for_lecture_notes,
)
from repositories.pages import (
    ...
    get_page_image_row,
    list_all_pages_for_lecture_notes_queue,
    list_document_pages_with_blocks_and_chat,
    list_page_image_paths_for_document,
    list_pages_without_lecture_notes_for_queue,
)
...
def read_document_status(document_id: str) -> dict[str, Any]:
    ...
    lecture_notes_ready_count = sum(
        1 for row in page_rows if row["lecture_notes"] and row["lecture_notes"].strip()
    )
    lecture_notes_failed_count = sum(
        1 for page in pages if page["lecture_notes_status"] == "failed"
    )
    lecture_notes_queue_counts = count_lecture_notes_queue_by_status(document_id)
    lecture_notes_pending_count = lecture_notes_queue_counts["waiting"]
    lecture_notes_processing_count = lecture_notes_queue_counts["processing"]

    lecture_notes_paused = bool(document["lecture_notes_paused"])
    # 正在生成的页面需要继续轮询到结束；暂停只阻止继续消费 waiting 队列。
    should_poll = (
        document["status"] == "processing"
        or document["course_summary_status"] == "processing"
        or lecture_notes_processing_count > 0
        or (not lecture_notes_paused and lecture_notes_pending_count > 0)
    )
...
@router.delete("/api/documents/{document_id}/lecture-notes/queue")
def clear_document_lecture_notes_queue(document_id: str) -> dict[str, str | int]:
    """清空指定文档尚未开始处理的讲稿生成队列。"""
    ...
    cleared_count = clear_waiting_lecture_notes_queue(document_id)
    return {
        "status": "cleared",
        "document_id": document_id,
...
def delete_document(document_id: str) -> Response:
    ...
    chat_attachment_paths = [
        resolve_chat_attachment_path(file_path)
        for file_path in list_chat_attachment_paths_for_document(document_id)
    ]
...
async def upload_document(
    ...
    if document_after_parse is not None and document_after_parse["status"] == "failed":
        update_course_summary_status(
            document_id=document_id,
